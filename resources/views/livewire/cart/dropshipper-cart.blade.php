<div class="min-h-screen bg-[var(--store-bg)]">
    @if($store->status !== \App\Enums\Status::CLONED)
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-32">
            <div class="bg-white rounded-3xl p-8 sm:p-12 text-center border border-stone-200/80 shadow-sm relative overflow-hidden group">
                ...
                <div class="absolute -right-12 -top-12 w-48 h-48 bg-rose-500/5 rounded-full blur-2xl pointer-events-none"></div>
                ...
                <div class="relative z-10 flex flex-col items-center">
                    <div class="w-14 h-14 rounded-2xl bg-rose-50 text-rose-500 flex items-center justify-center text-xl mb-6 border border-rose-100 shadow-xs">
                        <i class="fa-solid fa-store-slash"></i>
                    </div>

                    <span class="inline-flex items-center gap-1.5 bg-rose-50 text-rose-600 text-[10px] uppercase tracking-widest font-bold px-3 py-1 rounded-md mb-3 border border-rose-100">
                        Store Temporarily Unavailable
                    </span>

                    <h1 class="text-2xl sm:text-3xl font-light tracking-tight text-stone-900 mb-3">
                        <span class="font-medium text-stone-950">{{ $store->store_name }}</span> is currently inactive
                    </h1>

                    <p class="text-xs sm:text-sm text-stone-500 leading-relaxed max-w-md mb-8">
                        This store has been suspended by the administrator or the owner. Please reach out directly to <span class="font-semibold text-stone-800">{{ $store->dropshipper->user->name ?? 'the store owner' }}</span> for more information.
                    </p>
                    ...
                    @php
                        ...
                        $whatsappUrl = $cleanPhone ? "https://wa.me/{$cleanPhone}?text={$whatsappMessage}" : '#';
                    @endphp
                    ...
                    <div class="flex items-center justify-center gap-3 w-full">
                        @if($cleanPhone)
                            <a
                                ...
                                class="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-3 rounded-xl font-medium text-xs transition-all duration-150 flex items-center justify-center gap-2 shadow-sm"
                            >
                                <i class="fa-brands fa-whatsapp text-sm"></i>
                                <span>Chat on WhatsApp</span>
                            </a>
                        @else
                            <span class="text-xs text-stone-400 italic">No contact phone number available.</span>
                        @endif
                    </div>
                </div>
            </div>
    ...
    @else
        <div class="py-8 sm:py-12 text-stone-900">
