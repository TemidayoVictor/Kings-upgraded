<div class="min-h-screen bg-[#f7f5f2]">
    ...
    <div class="bg-[#e7dfd7] mx-4 sm:mx-6 lg:mx-8 mt-8 rounded-[32px] p-8 md:p-12 lg:p-16 relative overflow-hidden">
        ...
        <div class="relative max-w-2xl">
            <span class="text-sm uppercase tracking-[2px] text-[#6b4f40] mb-4 block">olive + botanicals</span>
            <h1 class="text-4xl md:text-5xl lg:text-6xl font-light text-[#1e1b1b] leading-tight">
                potion for the <span class="text-[#b55a3b] font-medium">modern skin</span>
            </h1>
            <p class="text-lg md:text-xl text-[#3f332c] mt-6 max-w-lg">
                radiant serums, cold‑pressed elixirs — formulated for glow.
            </p>
            <button class="mt-8 bg-[#1e1b1b] text-white px-8 py-4 rounded-full hover:bg-[#b55a3b] transition-all duration-300 shadow-lg hover:shadow-xl hover:-translate-y-1 flex items-center gap-2">
                shop bestsellers
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div class="flex flex-wrap justify-center gap-3 mb-12">
            ...
            @foreach($sections as $section)
                ...
                >
                    <i class="fa-regular fa-leaf text-[#b55a3b] mr-2"></i>
                    {{ $section->name }}
                </button>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 lg:gap-8">
            @forelse($products as $product)
                ...
                >
                    ...
                    <!-- Product Info -->
                    <div class="p-5">
                        <div class="text-xs uppercase tracking-wider text-[#b55a3b] mb-2">{{ $product->section->name ?? 'Uncategorized' }}</div>
                        <h3 class="text-lg font-medium text-[#1e1b1b] mb-2 line-clamp-1">{{ $product->name }}</h3>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-16">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 bg-[#eae1d7] rounded-[40px] p-8 md:p-12 border border-[#dfcfc0]">
            <div class="text-center">
                ...
                <h4 class="font-medium text-[#241f1b]">free shipping</h4>
                <p class="text-sm text-[#4c3f37] opacity-80">on orders over $65</p>
            </div>
            <div class="text-center">
                <i class="fa-regular fa-leaf text-3xl text-[#b55a3b] mb-3"></i>
                <h4 class="font-medium text-[#241f1b]">clean ingredients</h4>
                <p class="text-sm text-[#4c3f37] opacity-80">responsibly sourced</p>
            </div>
            <div class="text-center">
                <i class="fa-regular fa-rotate-left text-3xl text-[#b55a3b] mb-3"></i>
                <h4 class="font-medium text-[#241f1b]">30 day returns</h4>
                <p class="text-sm text-[#4c3f37] opacity-80">no questions asked</p>
            </div>
            <div class="text-center">
                <i class="fa-regular fa-gift text-3xl text-[#b55a3b] mb-3"></i>
                <h4 class="font-medium text-[#241f1b]">gift ready</h4>
                <p class="text-sm text-[#4c3f37] opacity-80">complimentary wrapping</p>
            </div>
        </div>
    </div>
            ...
            <div class="relative bg-white rounded-[32px] max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                <div class="grid md:grid-cols-2 gap-8 p-8">
                    ...
                    <div class="aspect-square bg-[#f8f3ef] rounded-2xl overflow-hidden p-8 flex items-center justify-center">
                        @if($quickViewProduct->cover_image)
                            <img src="{{ $quickViewProduct->thumbnail }}" alt="{{ $quickViewProduct->name }}" class="w-full h-full object-contain">
                        @endif
                    </div>

                    <!-- Details -->
                    <div>
                        <div class="text-sm uppercase tracking-wider text-[#b55a3b] mb-2">{{ $quickViewProduct->section->name ?? 'Uncategorized' }}</div>
                        <h2 class="text-3xl font-light text-[#1e1b1b] mb-4">{{ $quickViewProduct->name }}</h2>

                        <!-- Price -->
                        <div class="flex items-center gap-3 mb-6">
                            @if($quickViewProduct->discount_price)
                                <span class="text-3xl font-light text-[#1e1b1b]">₦{{ number_format($quickViewProduct->discount_price) }}</span>
                                <span class="text-lg text-[#94897f] line-through">₦{{ number_format($quickViewProduct->price) }}</span>
                                <span class="bg-[#b55a3b] text-white text-xs px-3 py-1.5 rounded-full">-{{ $quickViewProduct->discount_percentage }}%</span>
                            @else
                                <span class="text-3xl font-light text-[#1e1b1b]">₦{{ number_format($quickViewProduct->price) }}</span>
                            @endif
                        </div>

                        <!-- Description -->
                        <p class="text-[#4c3f37] leading-relaxed mb-8">{{ $quickViewProduct->description }}</p>
                        ...
                        @if($quickViewProduct->options)
                            @foreach($quickViewProduct->options as $optionName => $values)
                                <div class="mb-4">
                                    <label class="block text-sm text-[#94897f] mb-2 capitalize">{{ $optionName }}</label>
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($values as $value)
                                            <button class="px-4 py-2 bg-[#f8f3ef] text-[#2c2420] rounded-lg border border-[#e5dbd2] hover:border-[#b55a3b] transition-colors text-sm">
                                                {{ $value }}
                                            </button>
                                        @endforeach
                                    </div>
